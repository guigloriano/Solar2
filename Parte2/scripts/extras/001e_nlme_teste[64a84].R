...
caminho_dia64a84 <- paste(local_pasta, "datasets\\csv\\64_84.csv", sep = "")
dados_teste <- read.csv(caminho_dia64a84)
...
#  d3 <- NDA[xi[dia]:xf[dia],5]
  ...
  EQMi[i+63] <- round(mean((Dio - Dip)^2), 6)
...
##### 3.3 - Erro quadrático Médio Diário (p/ 84 dias) #####
#eqm <- numeric()
for(i in 1:(length(id.fi))){         
  dia <- i
  xi <- id.in[dia] 
  ...
  Dio <- NDA[xi:xf,6]
  Dip <- pred9[xi:xf]
  eqm[i+63] <- mean((Dio - Dip)^2)
}


nome_arquivo <- paste ("eqm1_", i+63, ".png", sep="")
pathDest <- paste(local_pasta, "graphs\\EQM\\", sep = "")
fileDest <- paste(pathDest, nome_arquivo, sep = "")
png(filename = fileDest, width = 720, height = 480, units = 'px')

sm <- seq(1,length(id.fi)+63)
plot(sm, eqm, type="l", xlim=c(0,85),  ylim=c(0,max(eqm)), ylab="EQM", xlab="Dia")
points(sm, eqm, pch=19)
dev.off()


##### 3.4 - Diferença Percentual Média (p/ 84 dias) ######   
#i <- 1 
#Dim <- numeric()
for(i in 1:length(id.fi)){
  Dio <- NDA[id.in[i]:id.fi[i],6]
  Dip <- as.numeric(pred9[id.in[i]:id.fi[i]])
  Dim[i+63] <- round(mean(( (exp(Dio) - exp(Dip) ) / exp(Dip) ) * 100), 6)
} 

###### 3.4.1 - Grafico em Barras das Diferenças Percentuais Medias #####

#### Informações para Salvar a imagem 
nome_arquivo <- paste ("dif_perc1_", i+63, ".png", sep="")
pathDest <- paste(local_pasta, "graphs\\DPM\\", sep = "")
fileDest <- paste(pathDest, nome_arquivo, sep = "")
png(filename = fileDest, width = 720, height = 480, units = 'px')


data_Bar2 <- data.frame (ListaDias_Bar = seq(1,length(id.fi)+63),
                         value = round(Dim, 4))
